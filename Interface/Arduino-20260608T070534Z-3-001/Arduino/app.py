...
ANTHROPIC_API_KEY   = ""
GEMINI_API_KEY      = "AQ.Ab8RN6KzuAssJXf-y9hgmchgCMTz-wIdxaRJyh4sMeURBHHGxQ"
...
class ArduinoVisionApp(tk.Tk):
    ...
    def _droidcam_reader(self, stream_url: str):
        """
        Read MJPEG stream via OpenCV; falls back to JPEG polling.
        FIX: after every successful JPEG poll, force cam_frame_queue refresh.
        """
        cap = cv2.VideoCapture(stream_url)
        if cap.isOpened():
            self.after(0, lambda: self.droidcam_status.configure(fg=SUCCESS))
            self.after(0, lambda: self._ai_log_append(
                "system", f"DroidCam MJPEG stream connected: {stream_url}\n"))
            while self.droidcam_running:
                ...
                # ── FIX: always push frame to queue and force display ──────
                try:
            ...
            # ── JPEG polling fallback ─────────────────────────────────────
            ip   = getattr(self, "_droidcam_ip",   "192.168.1.1")
            port = getattr(self, "_droidcam_port", "4747")
            snap_url = f"http://{ip}:{port}/shot.jpg"
            self.after(0, lambda: self._ai_log_append(
                "system",
                f"OpenCV MJPEG failed — polling JPEG from {snap_url}\n"))
            ...
            while self.droidcam_running:
                try:
                    ...
                    pil.load()   # force decode now while BytesIO is valid
                    self._latest_cam_frame = pil

                    # ── FIX: evict stale frame then push new one ──────────
                    try:
                        self.cam_frame_queue.get_nowait()
                    except queue.Empty:
                        pass
                    ...
                    if consecutive_errors == 5:
                        self.after(0, lambda err=str(e): self._ai_log_append(
                            "error",
                            f"DroidCam JPEG polling error (5 in a row): {err}\n"
                            "Check that DroidCam app is open and the IP is correct.\n\n"))
                time.sleep(0.15)   # ← slightly faster polling than before

        if self.droidcam_running:
            self.after(0, lambda: self._ai_log_append(
                "error", "DroidCam stream ended. Check IP and app.\n\n"))
